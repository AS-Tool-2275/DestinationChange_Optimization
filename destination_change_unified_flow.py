...
def apply_zero_ss_equalization(detail_full: pd.DataFrame) -> pd.DataFrame:
    """
    Fallback pass for items that have 2+ warehouses with Average of SS Wk3 = 0.
    It keeps total firm quantity unchanged and redistributes F Wk3 among
    zero-SS warehouses so their SI After values are closer together.
    """
    ...
    for col in ["Average of SS Wk3", "F Wk3 Original", "F Wk3 After Destination Change",
                "Net Destination Change", "Current SI After", "SS % After"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    if "Average of SS Wk3" not in df.columns or "Current SI After" not in df.columns:
        return df

    for _, g in df.groupby("Item", sort=False):
        zero_idx = [i for i in g.index if float(df.at[i, "Average of SS Wk3"] or 0) <= 0]
        if len(zero_idx) < 2:
            continue
        ...
        while guard < 100000:
            guard += 1
            current_si = df.loc[zero_idx, "Current SI After"].astype(float)
            donor = current_si.idxmax()
            recipient = current_si.idxmin()
            ...
            if int(round(float(df.at[donor, "F Wk3 After Destination Change"]))) <= 0:
                break

            df.at[donor, "F Wk3 After Destination Change"] = int(round(float(df.at[donor, "F Wk3 After Destination Change"]))) - 1
            df.at[recipient, "F Wk3 After Destination Change"] = int(round(float(df.at[recipient, "F Wk3 After Destination Change"]))) + 1

            df.at[donor, "Net Destination Change"] = int(round(float(df.at[donor, "Net Destination Change"]))) - 1
            df.at[recipient, "Net Destination Change"] = int(round(float(df.at[recipient, "Net Destination Change"]))) + 1

            df.at[donor, "Current SI After"] = donor_si - 1
            df.at[recipient, "Current SI After"] = recipient_si + 1

            donor_ss = float(df.at[donor, "Average of SS Wk3"])
            recipient_ss = float(df.at[recipient, "Average of SS Wk3"])
            df.at[donor, "SS % After"] = safe_ss_ratio(float(df.at[donor, "Current SI After"]), donor_ss)
            df.at[recipient, "SS % After"] = safe_ss_ratio(float(df.at[recipient, "Current SI After"]), recipient_ss)
