...
def build_detail_output(detail: pd.DataFrame) -> pd.DataFrame:
    ...
    preferred = [
        ...
        "Sub Vendor SI After", "Sub Vendor SS% After", "Sub Vendor DC Note",
        "Main Vendor", "Vendor",
    ]
...
def _allocate_secondary_vendor_greedy(detail_full: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, g in df.groupby("Item", sort=True):
        ...
        if total > 0:
            base_si = pd.to_numeric(g["Current SI After"], errors="coerce").fillna(0.0).to_numpy()
            ss = pd.to_numeric(g["Average of SS Wk3"], errors="coerce").fillna(0.0).to_numpy()
            whses = g["Whse"].astype(str).tolist()
            for _unit in range(total):
                candidates = []
                for i in range(len(g)):
                    si_after = base_si[i] + (final[i] - orig[i])
                    ratio = safe_ss_ratio(float(si_after), float(ss[i]))
                    candidates.append((ratio, -float(ss[i]), whses[i], i))
                candidates.sort(key=lambda x: (x[0], x[1], x[2]))
                final[candidates[0][3]] += 1
    ...
    df["Sub Vendor SI After"] = pd.to_numeric(df["Current SI After"], errors="coerce").fillna(0.0) + df["Sub Vendor Net Destination Change"]
...
def _osqp_equalize_single_item(F_orig: np.ndarray, curr_si: np.ndarray, avg_ss: np.ndarray) -> Tuple[Optional[np.ndarray], str]:
    ...
    n = len(F_orig)
    total_f = int(round(float(F_orig.sum())))
    if n <= 1 or total_f <= 0:
...
def build_osqp_sheets(detail_full: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    for _, g0 in detail_full.groupby("Item", sort=True):
        ...
        sx, sstatus = _osqp_equalize_single_item(sub_orig, sub_curr_si, avg_ss)
        if sx is None:
        ...
        for i in range(len(g)):
            snet = int(sx[i]) - int(round(sub_orig[i]))
            ...
            sub_records.append({
                ...
                "Sub Vendor F Original": int(round(sub_orig[i])),
...
def run_optimizer(
    ...
    detail_full = _allocate_secondary_vendor_greedy(detail_full)
...
def write_excel_output(
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        logic = pd.DataFrame([
            ...
            ["SI After", "= Current SI + Net Destination Change"],
            ["SS % After", "= SI After / Average of SS Wk3"],
            ["Sub Vendor DC columns", "Suggestion only. Starts after main-vendor destination change and redistributes Other Vendor Supply."],
