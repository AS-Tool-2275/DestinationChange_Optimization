...
def apply_zero_ss_equalization(detail_full: pd.DataFrame) -> pd.DataFrame:
    ...
    If an item has 2+ warehouses with Average of SS Wk3 == 0, we balance those
    warehouses by SI After (not SS%), while preserving the total firm quantity.
    This is intentionally a second pass after the normal optimizer result.
    """
    if detail_full is None or detail_full.empty:
        return detail_full

    df = detail_full.copy()
    for col in ["Average of SS Wk3", "F Wk3 Original", "F Wk3 After Destination Change", "Net Destination Change", "Current SI After", "SS % After"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    if "Item" not in df.columns or "Average of SS Wk3" not in df.columns:
        return df

    for _, g in df.groupby("Item", sort=False):
        zero_idx = g.index[g["Average of SS Wk3"].fillna(0).eq(0)].tolist()
        if len(zero_idx) < 2:
            continue

        # Move 1 unit from the highest-SI zero-SS warehouse to the lowest-SI zero-SS warehouse
        # until the spread is small or no more movement is possible.
        guard = 0
        while guard < 10000:
            guard += 1
            ...
            ss_donor = float(df.at[donor, "Average of SS Wk3"])
            ss_recipient = float(df.at[recipient, "Average of SS Wk3"])
            df.at[donor, "SS % After"] = safe_ss_ratio(float(df.at[donor, "Current SI After"]), ss_donor)
            df.at[recipient, "SS % After"] = safe_ss_ratio(float(df.at[recipient, "Current SI After"]), ss_recipient)
