...
def detect_timeline_vendors(plan_csv_path: str) -> pd.DataFrame:
    """Detect unique vendor order from PlanDetailTimeline.

    Vendor names are not resolved here. The function only extracts the vendor
    code / key in the order of first appearance so the UI can show DueDateCalc
    upload order.
    """
...
def detect_psw_vendors(psw_csv_paths: List[str]) -> pd.DataFrame:
    ...
    This version intentionally uses vendor codes only; it does not resolve or
...
def build_vendor_offset_maps(
    ...
    The mapping follows the detected vendor order. If fewer DueDateCalc files
    are uploaded than vendors detected, the last uploaded file is reused as
    fallback. This function works with vendor codes only.
    """
    ...
    if timeline_vendor_df is None or timeline_vendor_df.empty:
        return vendor_maps, pd.DataFrame([["Vendor DueDate mapping", "No vendor detected; default DueDateCalc will be used"]], columns=["Field", "Value"])

    if not due_paths:
        return vendor_maps, pd.DataFrame([["Vendor DueDate mapping", "No DueDateCalc files provided for vendor mapping"]], columns=["Field", "Value"])
...
def process_files(
    ...
    timeline_vendor_df = detect_timeline_vendors(plan_detail_csv)
    vendor_offset_maps, vendor_due_debug = build_vendor_offset_maps(timeline_vendor_df, due_list, offset_mode=offset_mode)
