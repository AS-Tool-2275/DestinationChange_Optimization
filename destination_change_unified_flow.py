...
# Fallback equalization pass for zero-SS warehouses.
# This keeps the main allocation logic unchanged, but balances warehouses with
# Average of SS Wk3 = 0 by comparing SI After only.
_fallback_equalize_zero_ss_rows(rows)
    ...
    if int(out["F Wk3 Original"].sum()) != int(out["F Wk3 After Destination Change"].sum()):
        raise ValueError(f"Item {out['Item'].iloc[0]}: firm total is not preserved.")
    return out
...
def apply_zero_ss_equalization(detail_full: pd.DataFrame) -> pd.DataFrame:
    """
    Fallback equalization pass for items with 2+ warehouses where Average of SS Wk3 = 0.
    It only rebalances firm units among zero-SS warehouses so their SI After values
    become closer to each other, while preserving the item total firm quantity.
    """
    ...
    needed_cols = ["Item", "Whse", "Average of SS Wk3", "F Wk3 Original", "F Wk3 After Destination Change", "Net Destination Change", "Current SI After", "SS % After"]
    if any(c not in df.columns for c in needed_cols):
        return df

    for col in ["Average of SS Wk3", "F Wk3 Original", "F Wk3 After Destination Change", "Net Destination Change", "Current SI After"]:
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0.0)

    for item, g in df.groupby("Item", sort=False):
        zero_idx = g.index[g["Average of SS Wk3"] <= 0].tolist()
        if len(zero_idx) < 2:
            continue

        while True:
            current = df.loc[zero_idx, "Current SI After"].astype(float)
            donor = current.idxmax()
            recipient = current.idxmin()
