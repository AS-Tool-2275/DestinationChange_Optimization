...
st.caption("Multi-vendor PSW support with firm PO reconciliation, sub-vendor suggestions, and vendor-name preview in the mapping table")
...
with col1:
    ...
    due_files = st.file_uploader(
        ...
        key="due",
        help=(
            "Upload order should match the vendor order detected from PSW / Production Schedule. "
...
# Preview vendor order from PSW / Production Schedule so users can upload matching DueDateCalc files.
if psw_files:
    try:
        with tempfile.TemporaryDirectory() as vendor_tmp:
            ...
            plan_vendor_df = pd.DataFrame()
            if plan_file is not None:
                preview_plan_path = Path(vendor_tmp) / Path(plan_file.name).name
                preview_plan_path.write_bytes(plan_file.getvalue())
                try:
                    plan_vendor_df = detect_timeline_vendors(str(preview_plan_path))
                except Exception:
                    plan_vendor_df = pd.DataFrame()

            vendor_preview_df = build_vendor_display_preview(vendor_preview_df, plan_vendor_df)

        if not vendor_preview_df.empty:
            st.subheader("Detected vendor order for DueDateCalc mapping")
            st.caption(
                "Vendor list is detected from PSW / Production Schedule. Upload DueDateCalc files in this order. "
            ...
            st.dataframe(vendor_preview_df, width='stretch', hide_index=True)
            if due_files:
...
def prettify_vendor_label(value: object) -> str:
    text = "" if value is None else str(value).strip()
    if not text or text.lower() == "nan":
        return ""
    text = re.sub(r"\s*\(\s*0*\d+\s*\)\s*$", "", text).strip()
    return text


def build_vendor_display_preview(psw_vendor_df: pd.DataFrame, plan_vendor_df: pd.DataFrame | None = None) -> pd.DataFrame:
    if psw_vendor_df is None or psw_vendor_df.empty:
        return psw_vendor_df
    df = psw_vendor_df.copy()
    df["Vendor"] = df["Vendor"].map(prettify_vendor_label)

    if plan_vendor_df is not None and not plan_vendor_df.empty and "Vendor Code" in plan_vendor_df.columns and "Vendor" in plan_vendor_df.columns:
        plan_map = (
            plan_vendor_df[["Vendor Code", "Vendor"]]
            .dropna(subset=["Vendor Code"])
            .drop_duplicates(subset=["Vendor Code"], keep="first")
            .rename(columns={"Vendor": "Plan Vendor"})
        )
        df = df.merge(plan_map, on="Vendor Code", how="left")
        df["Vendor"] = df["Plan Vendor"].where(df["Plan Vendor"].notna() & (df["Plan Vendor"].astype(str).str.strip() != ""), df["Vendor"])
        df["Vendor"] = df["Vendor"].map(prettify_vendor_label)
        df = df.drop(columns=["Plan Vendor"], errors="ignore")
    return df
...
if st.button("Run Full Flow", type="primary"):
    ...
    if not due_files:
        ...
        st.stop()
    # Multiple DueDateCalc files are supported; upload order follows the vendor order detected from PSW.
