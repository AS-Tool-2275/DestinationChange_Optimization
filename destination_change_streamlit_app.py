...
st.caption("Multi-vendor PSW support with firm PO reconciliation and sub-vendor suggestions.")
...
if psw_files:
    try:
        ...
        if not vendor_preview_df.empty:
            ...
            st.caption(
                "Vendor order is detected from PSW / Production Schedule vendor codes only. "
                "Upload DueDateCalc files in this order. Example: vendor order #1 uses DueDateCalc file #1; "
                "vendor order #2 uses DueDateCalc file #2. If one DueDateCalc is shared by all vendors, upload only one file."
            )
            ...
            st.warning("No vendor code was detected from PSW. The app will use the first DueDateCalc as default transit time.")
